  ...
  if (guestOrderConfirmation) {
    return (
      <div className="mt-16 text-center px-4">
        <div className="max-w-md mx-auto bg-primary-dull border border-secondary/20 rounded-2xl p-6 sm:p-8">
          <div className="w-14 h-14 mx-auto rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center text-accent text-2xl">
            ✓
          </div>
          ...
          <button
            ...
            className="mt-6 w-full bg-accent hover:bg-accent-dull text-primary font-semibold py-2.5 rounded-lg transition-colors"
          >
  ...
  if (cartArray.length === 0) {
    return (
      <div className="mt-16 text-center text-gray-500 px-4">
        ...
        <button
          onClick={() => navigate("/products")}
          className="mt-4 px-4 py-2 bg-primary hover:bg-primary-dull text-white rounded-lg text-sm sm:text-base transition-colors"
        >
          {t("cart.continue_shopping")}
        </button>
      </div>
            ...
            >
              <div className="flex gap-3 sm:gap-4">
                <div
                  onClick={() => {
                    navigate(
                      `/products/${product.category.toLowerCase()}/${product.slug || product._id}`
                    );
                    scrollTo(0, 0);
                  }}
...
/>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <div className="flex items-center border border-gray-300 rounded-md overflow-hidden">
                        <button
                          ...
                          className="w-7 h-7 sm:w-6 sm:h-6 flex items-center justify-center text-gray-600 hover:bg-gray-100 disabled:opacity-40 disabled:hover:bg-transparent disabled:cursor-not-allowed transition-colors cursor-pointer"
                        >
                          −
                        </button>
        ...
        <button
          onClick={() => navigate("/products")}
          className="mt-6 sm:mt-8 text-secondary hover:text-secondary-dull text-sm sm:text-base transition-colors"
        >
      ...
      <div className="w-full lg:max-w-[380px] lg:shrink-0">
        <div className="relative bg-primary-dull border border-secondary/20 rounded-2xl p-5 sm:p-6 shadow-[0_0_40px_rgba(0,0,0,0.5)] overflow-hidden lg:sticky lg:top-24">
          <div className="absolute -top-20 -right-20 w-48 h-48 bg-accent/20 blur-3xl rounded-full pointer-events-none" />

          <h2 className="text-base sm:text-lg font-bold text-white tracking-wide flex items-center gap-2">
            {t("cart.order_summary")}
          </h2>
          <div className="h-px bg-gradient-to-r from-accent/60 via-white/10 to-transparent mt-3 mb-5" />
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 bg-white/[0.03] border border-white/10 rounded-lg p-3">
                ...
                <button
                  ...
                  className="text-accent text-xs font-semibold mt-2 hover:text-accent-light transition-colors"
                >
              ...
              {showAddress && (
                <div className="mt-2 bg-primary border border-white/10 rounded-lg overflow-hidden max-h-48 overflow-y-auto">
                  {addresses.length > 0 ? (
                    addresses.map((addr) => (
                      <p
                        key={addr._id}
                        ...
                        className="px-3 py-2 hover:bg-white/5 cursor-pointer text-sm text-gray-300 border-b border-white/5 last:border-0 transition-colors"
                      >
                  ...
                  <p
                    ...
                    className="px-3 py-2 text-accent cursor-pointer text-sm font-medium hover:bg-white/5 transition-colors"
                  >
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 space-y-2">
                <input
                  ...
                  className="w-full bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-sm text-gray-200 placeholder:text-gray-600 outline-none focus:border-accent/60 transition-colors"
                />
              ...
              <button
                ...
                className={`py-2 rounded-lg text-xs sm:text-sm font-medium border transition-all duration-200 ${
                  paymentOption === "COD"
                    ...
                    : "border-white/10 text-gray-500 bg-transparent hover:border-accent/50 hover:text-accent-light hover:bg-accent/5"
                }`}
              >
                {t("cart.cod")}
              </button>
          ...
          <button
            onClick={placeOrder}
            disabled={loading}
            className="w-full mt-4 bg-accent hover:bg-accent-dull text-primary font-semibold py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_20px_rgba(255,226,0,0.25)] text-sm sm:text-base"
          >
            {loading
              ? t("cart.processing")
              : paymentOption === "COD"
